...
- **Page layout preserved.** Today section, `RadialCompletion`, and `HabitHistory` are untouched. New composition: Today → (Momentum 2/3 + Radial 1/3 grid) → Streaks → History. `allRegistryResult` query dropped (only the heatmap needed inactive habits for name lookup).
