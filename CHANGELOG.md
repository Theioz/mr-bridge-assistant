...
### Fixed
- **Strict-mode tool schemas tightened to satisfy Anthropic's `additionalProperties: false` requirement (#343 follow-up).** PR #359 enabled `strict: true` on five mutating tools without tightening the JSON schemas underneath. Every chat request to Anthropic was then failing with `400 invalid_request_error: tools.N.custom: For 'object' type, 'additionalProperties' must be explicitly set to false` — surfaced in the browser as the generic `Error — try again` banner on the first user message of every session. Local `npm run dev` would have reproduced this on the first chat turn; I skipped the smoke-start step from the plan, so the defect reached prod. Fix: add `additionalProperties: false` to every `object` schema level across the five strict tools ([profile.ts](web/src/lib/tools/profile.ts) ×2, [calendar.ts](web/src/lib/tools/calendar.ts) ×2, [workouts.ts](web/src/lib/tools/workouts.ts) — `assign_workout` top-level + 3 exercise-array item schemas, `update_workout_exercise` top-level + `updates` nested object). `assign_workout.warmup/workout/cooldown` also grew full `items` property shapes (`exercise` required; `sets`, `reps`, `weight_lbs`, `notes` optional) — previously `items: { type: "object" }` with no properties, which strict mode rejects outright. `update_workout_exercise.updates` gained its own `additionalProperties: false`. Strict gates remain enabled; no rollback needed.
