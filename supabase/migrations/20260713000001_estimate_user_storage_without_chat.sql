-- Recreate estimate_user_storage() without the chat domain (#611).
--
-- The previous definition counted rows and bytes from chat_messages / chat_sessions for
-- the Settings -> Usage panel. Those tables are dropped in the companion migration, so
-- this function would throw at runtime the next time anyone opened Settings. Recreate it
-- without the chat block rather than leave a landmine.
